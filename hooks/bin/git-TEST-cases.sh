#!/bin/bash
# 	hooks/bin/git-TEST-cases.sh  2.432.1323  2020-01-27T23:45:16.249962-06:00 (CST)  https://github.com/BradleyA/git-TEST-commit-automation.git  master  uadmin  five-rpi3b.cptx86.com 2.431  
...
if [[ "${REPOSITORY_DIR}" == ""  ]] ; then
  REPOSITORY_DIR=$(git rev-parse --show-toplevel) || { new_message "${LINENO}" "${RED}ERROR${WHITE}" "  git-TEST-cases.sh: Not a git repository (or any of the parent directories)" 1>&2 ; exit 1 ; } 
fi
cd "${REPOSITORY_DIR}"
