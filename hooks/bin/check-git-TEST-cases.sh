#!/bin/bash
# 	hooks/bin/list-git-TEST-cases.sh  2.2.270  2019-09-01T13:38:43.648913-05:00 (CDT)  https://github.com/BradleyA/git-TEST-commit-automation.git  uadmin  five-rpi3b.cptx86.com 2.1  
# 	   hooks/bin/list-git-TEST-cases.sh  change tab to two spaces update Production standard 1.3.475 DEBUG variable  change SAST to SA #1 
...
###   hooks/bin/list-git-TEST-cases.sh -  search from top of repository to list TEST directory test cases
...
for i in $DIR_LIST ; do
  ...
  for j in ${TEST_CASE_DIR_LIST} ; do 
    ...
    if [[ "${1}" == "clean" ]]  ; then cd "${REPOSITORY_DIR}/${j}" ; for k in $(ls -1) ; do { [ ! -L "${k}" ] || rm -v "${k}"; } ; rm -f FVT-*.test-case-output ; rm -f SA-*.test-case-output ; done ; cd "${REPOSITORY_DIR}" ; fi
